...
class EventCfg:
    ...
    # reset_arm_joints_random =EventTerm(
    #     func=mdp.reset_arm_joints_random,
    #     mode="reset",
    #     params={"asset_cfg": SceneEntityCfg("robot", joint_names=".*_arm_.*"),  # 机械臂关节ID
    #         "position_range": (-3.14, 3.14),  # ±180度范围内随机
    ...
    # randomize_arm_joint_targets =EventTerm(
...
class RewardsCfg:
    ...
    undesired_contacts = RewTerm(
        func=mdp.undesired_contacts,                                 # 惩罚非足端部位接触地面
        weight=0.0,
        params={
            "sensor_cfg": SceneEntityCfg("contact_forces", body_names=""),
            "threshold": 1.0,
    ...
    contact_forces = RewTerm(                                        # 惩罚足端接触力过大
...
class TerminationsCfg:
    ...
    # base_tilt = DoneTerm(
    #     func=mdp.base_tilt_termination,
    #     params={
    #         "asset_cfg": SceneEntityCfg("robot", body_names=""),
    #         "max_tilt_angle": 60.0,  # 60度
    #     },
    #     time_out=False,  # 这不是超时，是失败
    # )
...
@configclass
class QUA_ENGINEERLocomotionVelocityRoughEnvCfg(ManagerBasedRLEnvCfg):
